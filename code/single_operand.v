		...
		data_type=instruction[15];
          ...
          PSW[NEGATIVE] = 0;
				...
				begin
					...
					result = ~read(instruction[5:3],instruction[2:0],data_type);
          	...
          	PSW[NEGATIVE] = 1;
				...
				begin
					//$display("%b",~read(instruction[5:3],instruction[2:0]));
					result = -(read(instruction[5:3],instruction[2:0],data_type)); //- works ? Best thing is to do 2's complement and store
					if(write(instruction[5:3],instruction[2:0],result,data_type))
          	...
          	PSW[NEGATIVE] = 1;
					...
					TST:
				begin
					result = read(instruction[5:3],instruction[2:0],data_type) - 0;
					if(result == 0)
						PSW[ZERO] = 1;
					else
						PSW[ZERO] = 0;
